...
| 2026-02-25 | Plan review: Fixed constant names (`SEGMENTFLOW_PATH`/`SEGMENTFLOW_URL`). Added cross-repo coordination section for Phase 2. Added `src/global.d.ts` type declarations. Fixed tsdown `globalName` conflict. Added `register_hooks()` wiring instructions. Expanded Phase 4 test structure. Resolved all 3 open questions. Documented known limitations (`already_tracked` race condition, Elementor Pro `fetch` API). Added edge cases for ad blockers. |
...
| File                                                         | Change                                                                             |
| ------------------------------------------------------------ | ---------------------------------------------------------------------------------- |
| `integrations/woocommerce/class-segmentflow-wc-tracking.php` | Add `inject_page_data()` method + private helpers, hook to `wp_head` at priority 0 |
| `integrations/woocommerce/class-segmentflow-wc-helper.php`   | Add helper methods for product/cart/order data extraction                          |

### Wiring: `register_hooks()` Update

The existing `register_hooks()` method in `class-segmentflow-wc-tracking.php` currently only adds the context filter. Add the new `wp_head` action:

```php
public function register_hooks(): void {
    // Existing: enrich SDK context with WooCommerce data.
    add_filter( 'segmentflow_tracking_context', [ $this, 'add_woocommerce_context' ] );

    // New: inject window.__sf_wc page data before the SDK loads (priority 0 < SDK priority 1).
    add_action( 'wp_head', [ $this, 'inject_page_data' ], 0 );
}
```

**Note**: The class constructor receives both `Segmentflow_Options` and `Segmentflow_Tracking` instances (matching the existing pattern in `includes/class-segmentflow.php:97-99`). The new `inject_page_data()` method doesn't use either dependency directly — it calls WooCommerce global functions — but they're available if needed.
...
**Repository**: This phase is implemented in the **segmentflow-ai** repo, not segmentflow-connect.

### Cross-Repo Coordination

Phase 2 modifies the CDN SDK (`segmentflow-ai` repo), while Phases 1/3 modify this plugin (`segmentflow-connect`). The release order matters:

1. **Phase 1 (this repo)** can ship first — it only sets `window.__sf_wc` which is harmless if no SDK reads it yet.
2. **Phase 2 (segmentflow-ai)** must be deployed to CDN before users see client-side events. The updated SDK reads `window.__sf_wc` and fires track events.
3. **Phase 3 (this repo)** is independent — `storefront.ts` calls `window.segmentflow?.track()` directly and doesn't depend on the WooCommercePlugin changes.

**Backward compatibility**: If a user has the new plugin (Phase 1) but the old CDN SDK (without Phase 2), `window.__sf_wc` is set but never read. No errors, no events — graceful degradation. When the CDN SDK updates, events start flowing automatically without a plugin update.

**Forward compatibility**: If a user has the old plugin (no `window.__sf_wc`) but the new CDN SDK (Phase 2), the SDK checks for `window.__sf_wc`, finds nothing, and exits. No errors.

### Files Modified

| File                                                                                      | Change                                                       |
| ----------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| `packages/browser/sdk-web/src/plugins/woocommerce.plugin.ts` (in **segmentflow-ai** repo) | Add page-load event firing + DOM event listeners in `load()` |
...
const $ = (window as any).jQuery;
if ($) {
  $(document).ajaxComplete((_event: any, xhr: any, settings: { data?: string }) => {
...
The current `tsdown.config.ts` has a single entry (`src/admin.ts`) with `globalName: "SegmentflowAdmin"`. Adding a second entry requires removing `globalName` (it would incorrectly apply to both bundles). The admin bundle doesn't actually use the global name externally, so dropping it is safe. The storefront bundle is self-executing (event listeners) and needs no global name either.
...
**Important**: Verify that the existing admin page JS still works after removing `globalName: "SegmentflowAdmin"`. The admin script is self-executing (calls `init()` at the bottom) and is not referenced externally, so this should be safe.

### TypeScript Type Declarations

A new `src/global.d.ts` file is needed so that `storefront.ts` can reference `window.segmentflow` without TypeScript errors (the project uses `strict: true` and `noImplicitAny: true`):
...
/** WooCommerce page data injected by PHP (class-segmentflow-wc-tracking.php) */
interface SfWcPageData {
  page: "product" | "cart" | "checkout" | "thankyou" | "shop" | "category" | "other";
  currency: string;
  product?: {
...
public function enqueue_storefront_assets(): void {
    ...
    $script_path = SEGMENTFLOW_PATH . 'assets/js/storefront.js';
    if ( ! file_exists( $script_path ) ) return;

    wp_enqueue_script(
        'segmentflow-storefront',
        SEGMENTFLOW_URL . 'assets/js/storefront.js',
        [],
...
**PHP unit tests** (`tests/test-wc-tracking-data.php`):

Tests for the new PHP data extraction methods. Uses the existing `WP_UnitTestCase` pattern with WC active checks (consistent with `tests/test-tracking-woocommerce.php`). Key test cases:

| Test Method                             | What It Verifies                                                                                                  |
| --------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `test_get_page_type_product`            | Returns `"product"` on single product pages                                                                       |
| `test_get_page_type_cart`               | Returns `"cart"` on cart page                                                                                     |
| `test_get_page_type_checkout`           | Returns `"checkout"` on checkout (not order-received)                                                             |
| `test_get_page_type_thankyou`           | Returns `"thankyou"` on order-received endpoint                                                                   |
| `test_get_product_data_shape`           | Product data array has all required keys (`id`, `name`, `price`, `sku`, `categories`, `image_url`, `url`, `type`) |
| `test_get_product_data_null_no_product` | Returns `null` when global `$product` is not set                                                                  |
| `test_get_cart_data_shape`              | Cart data array has `items`, `total`, `subtotal`, `item_count`, `cart_hash`                                       |
| `test_get_cart_data_null_empty`         | Returns `null` when cart is empty                                                                                 |
| `test_get_order_data_shape`             | Order data has all required keys and correct types                                                                |
| `test_get_order_data_dedup_flag`        | First call sets `already_tracked: false`, second call sets `already_tracked: true`                                |
| `test_inject_page_data_renders_script`  | Output contains `<script>window.__sf_wc = ` with valid JSON                                                       |

**Note**: Page type detection tests require mocking WordPress conditional tags (`is_product()`, `is_cart()`, etc.). The existing test suite doesn't mock these — consider using `WC_Helper_Product` to create test products and `WC_Helper_Order` for test orders if available, or test the data extraction methods directly by passing mock objects.

**SDK tests** (`packages/browser/sdk-web/src/plugins/__tests__/woocommerce.plugin.test.ts` in **segmentflow-ai** repo):

Verify event firing based on mocked `window.__sf_wc` data. These are standard unit tests with a mock analytics instance.
...
| SDK blocked by ad blocker     | `order_completed` client event lost, but server-side webhook still fires. See note on `already_tracked` below            |
| Elementor Pro using fetch API | Newer Elementor Pro versions may use `fetch` instead of jQuery AJAX — `ajaxComplete` won't fire. Known limitation for v1 |
...
6. **Plugin constants** are `SEGMENTFLOW_PATH` (directory path) and `SEGMENTFLOW_URL` (URL) — defined in `segmentflow-connect.php`.
7. **Cross-repo deployment**: Phase 2 (SDK changes in segmentflow-ai) must be deployed to CDN before client-side WooCommerce events will fire. Phases 1 and 3 are safe to deploy independently.

---

## Known Limitations

These are accepted trade-offs for v1, documented here for future reference:

### `already_tracked` optimistic flag

The PHP `get_order_data()` method sets `_sf_order_tracked` meta optimistically on the first page load of the thank-you page, before the JS event actually fires. If the SDK is blocked (ad blocker, network error, JS error), the order is marked as tracked but the client-side event was never sent.

**Mitigation**: The server-side webhook fires `order_completed` independently with authoritative order data. The client-side event is additive (captures browser context like UTM, referrer, device) but not critical. Downstream deduplication uses `order_id`, so if only the webhook fires, no data is lost — only browser context is missing for that order.

**Alternative considered**: Having the SDK call back to WordPress via AJAX after successful `track()` to set the meta. Rejected for v1 because it adds complexity (AJAX endpoint, nonce handling) for a rare edge case. Revisit if ad blocker rates prove high.

### Elementor Pro `fetch` API

Newer versions of Elementor Pro may use the `fetch` API instead of jQuery AJAX for form submissions. The `jQuery.ajaxComplete` hook used in `storefront.ts` won't intercept these. This is a known limitation for v1. A future enhancement could use a `fetch` interceptor or listen for Elementor-specific DOM events.
...
### 1. Product impressions on shop/category pages

**Decision**: `product_viewed` fires only on single product pages for v1. Shop/category page product impressions (a "product_list_viewed" event) would require PHP to inject an array of all visible products, which varies by theme and pagination. Deferred to a future enhancement.

### 2. WooCommerce Blocks adoption

**Decision**: Accepted as a known gap for v1. Blocks cart/checkout uses React, not jQuery events. The `added_to_cart` / `removed_from_cart` jQuery events won't fire in Blocks-based pages. The email selector fallback (`input[id*="email"][autocomplete="email"]`) partially handles Blocks checkout for email capture. A future phase should add proper Blocks support using the `@woocommerce/blocks-checkout` extensibility API.

### 3. `sendBeacon` vs `analytics.flush()` for single-product add-to-cart

**Decision**: Use `analytics.flush()` for v1. The SDK's `flush()` method uses `fetch` with `keepalive: true` when available, which has similar reliability to `sendBeacon` for page-unload scenarios. If testing reveals dropped events, switch to calling `navigator.sendBeacon()` directly with the serialized event payload as a follow-up. The form submit listener should also be defensive — it calls `flush()` but doesn't `preventDefault()`, so the form submits normally regardless.
